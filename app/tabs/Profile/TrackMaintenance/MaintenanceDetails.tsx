...
export default function ViewPropertyDetails() {
  ...
  const { withdrawMaintenance } = useAuth();
                                ...
                                if (userData) {
                                    ...
                                    const unsubscribeProperty = onSnapshot(propertyRef, async (propertyDoc) => {
                                        if (propertyDoc.exists()) {
                                            ...
                                            if (propertyData) {
                                                const ownerImage = userData.profilePicture ? await getUserImageUrl(ownerId) : require('../../../../assets/images/property1.png');
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
                <View className='mb-28'>
                    ...
                    <View className='flex-row justify-between mt-4 mb-1 mx-4'>
                        <Text className='text-sm font-bold'>Property & Landlord Information</Text>
                    </View>
                                        ...
                                        <View className='flex-col'>
                                            ...
                                            <View className='flex-row space-x-2'>
                                                <TouchableOpacity onPress={async () => {
                                                    ...
                                                    await SecureStore.setItemAsync('messageRecipientId', maintenance?.ownerId ?? '');
                                                    }}>
        ...
        <View className='w-full flex-row items-center justify-between bg-white px-8 py-4 absolute bottom-5 border-t border-gray-200'>
            <View className='flex-col '>
                ...
                <View className='flex-row items-center space-x-2 r'>
                    ...
                    <Text numberOfLines={1} ellipsizeMode="tail" className="text-sm font-bold" 
                        ...
                        {maintenance?.maintenanceStatus === 'Approved' ? (
                            <Text>Maintenance Scheduled for {maintenance?.preferredTime}</Text>
                        ) : maintenance?.maintenanceStatus === 'Pending' ? (
                              <Text>Under Review</Text>
                        ) : (
                            <Text>{maintenance?.maintenanceStatus}</Text>
                        )}
                    </Text>
                </View>
            </View>
            ...
            <View className=' flex-row space-x-2'>
                ...
                (
                    <>
                        <TouchableOpacity
                          className="bg-[#D9534F] rounded-xl py-2 px-3"
                          onPress={() => setModalVisible(true)} // Open modal on press
                        >
                          <Text className="text-white text-xs font-bold">Withdraw Request</Text>
                        </TouchableOpacity>
                    </>
                )}
            </View>
...
{/* Modal for confirming withdrawal */}
        ...
        >
          <View className="flex-1 justify-center items-center bg-black/50">
            <View className="bg-white p-6 rounded-lg w-2/3 shadow-lg">
              <Text className="text-sm text-center font-bold mb-4">Withdraw Maintenance</Text>
              <Text className="mb-6 text-xs text-center">Are you sure you want to withdraw your maintenance for {maintenance?.ownerFullName}?</Text>
              <View className="flex flex-row justify-center gap-5">
                <TouchableOpacity
                  className="py-2 px-4 bg-[#D9534F] rounded-md"
                  onPress={handleWithdrawMaintenance}
                >
                  <Text className="text-white text-xs font-bold">Confirm</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  className="py-2 px-4 bg-[#333333] rounded-md"
                  onPress={() => setModalVisible(false)}
                >
                  <Text className="font-bold text-xs text-white">Cancel</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
