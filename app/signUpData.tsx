...
export default function SignUp() {
  ...
  const handleContinue = async () => {
    if (!firstName || !lastName || !birthday || !age || !selectedGender || !selectedEducation || !selectedCivilStatus) {
    ...
    try {
      ...
      await SecureStore.setItemAsync('middleName', middleName);
      await SecureStore.setItemAsync('lastName', lastName);
      await SecureStore.setItemAsync('birthday', birthday);
      await SecureStore.setItemAsync('age', age.toString());
      await SecureStore.setItemAsync('gender', selectedGender);
      await SecureStore.setItemAsync('education', selectedEducation);
      await SecureStore.setItemAsync('civilStatus', selectedCivilStatus);
  ...
  const onChange = (event: DateTimePickerEvent, selectedDate: Date | undefined) => {
    if (event.type === 'set') {
      ...
      setShow(false); // Close the DateTimePicker modal immediately after selecting a date
      setDate(currentDate);
      calculateAge(currentDate); // Calculate age when date is changed
    } else {
      setShow(false); // Close the DateTimePicker modal if the user cancels
    }
  };
  ...
  const calculateAge = (birthDate: Date) => {
    ...
    if (calculatedAge < 18) {
      Alert.alert('Sign Up', 'You must be at least 18 years old to register.');
      setBirthday('Calendar'); // Reset birthday display
      setAge(''); // Clear age display
    } else {
      setBirthday(birthDate.toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }));
    }
  };
              ...
              {genderDropdownOpen && (
                <View className='px-4 bg-gray-100 rounded-xl'>
                  {GENDER_OPTIONS.map((option, index) => (
                    <Pressable key={index} onPress={() => { setSelectedGender(option); setGenderDropdownOpen(false); }} className='py-2'>
              ...
              {civilStatusDropdownOpen && (
                <View className='px-4 bg-gray-100 rounded-xl'>
                  {CIVIL_STATUS_OPTIONS.map((option, index) => (
                    <Pressable key={index} onPress={() => { setSelectedCivilStatus(option); setCivilStatusDropdownOpen(false); }} className='py-2'>
                      <Text className='text-xs'>{option}</Text>
                    </Pressable>
                  ))}
                </View>
              ...
              {educationDropdownOpen && (
                <View className='px-4 bg-gray-100 rounded-xl'>
                  {EDUCATION_OPTIONS.map((option, index) => (
                    <Pressable key={index} onPress={() => { setSelectedEducation(option); setEducationDropdownOpen(false); }} className='py-2'>
                      <Text className='text-xs'>{option}</Text>
                    </Pressable>
                  ))}
                </View>
